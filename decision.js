var decision = {
	explicitCommands : [
		{ command: /^carbot!+\s?/, behavior: 'customPick' },
		{ command: /^카봇!+\s?/, behavior: 'customPick' },
		{ command: /^zkqht!+\s?/, behavior: 'customPick' },
	],
	keywordList : [
	...
	behaviors : {
		handleIrregulars : function(text) {
			var _ = decision;
			for (var i = 0; i < _.irregularTable.length; i++) {
				var regex = new RegExp(_.irregularTable[i].pattern);
				var match = text.match(regex);
				if (match === null) continue;
				return (match.length > 1 ? text.replace(regex, '$1' + _.irregularTable[i].fix) : text.replace(regex, _.irregularTable[i].fix));
			}
			return text;
		},
		pickOne: function(matchResult, list) {
			var response = list[Math.floor(Math.random() * list.length)].replace('$', matchResult[1]);
			return decision.behaviors.handleIrregulars(response)
		},
	...
	checkKeywordAndGetResponse : function(text) {
		if (text === undefined) return null;

		var _ = this;

		// Check explicit commands first
		for (var i = 0; i < _.explicitCommands.length; i++) {
			var match = text.trim().match(new RegExp(_.explicitCommands[i].command));
			if (match === null) continue;
			return _.behaviors[_.explicitCommands[i].behavior](match, _.explicitCommands[i].parameter);
		}

		// Check keywords
		for (var i = 0; i < _.keywordList.length; i++) {
			var match = text.trim().match(new RegExp(_.keywordList[i].keyword));
			if (match === null) continue;
			return _.behaviors[_.keywordList[i].behavior](match, _.keywordList[i].parameter);
		}
		return null;
	}
};
